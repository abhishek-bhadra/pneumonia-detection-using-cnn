...
#
lbl = tk.Label(root, text="Welcome to Pneumoina Detection", font=('times', 30,' bold underline '), width=70,height=1,bg="black",fg="white")
lbl.place(x=0, y=0)
...
def test_model_proc(fn):
    ...
    if fn!="":
        ...
        if yoga == 0:
            Cd="Pnemonia  not Detected"
            
        elif yoga == 1:
            Cd="Pnemonia Detected"
      
            
        A=Cd
        return A
    img = open(fn)    
    myfile = open("D:/22CB140-Pneumonia Detection/22CB140-Pneumonia Detection/training/1/1.txt")
    if img == myfile:
          print(myfile)
...
def openimage():
    ...
    global fn
    print(fn)
    fileName = askopenfilename(initialdir='D:/22CB140-Pneumonia Detection/22CB140-Pneumonia Detection', title='Select image for Aanalysis ',
                               filetypes=[("all files", "*.*")])
    IMAGE_SIZE=200
    imgpath = fileName
    fn = fileName
